...
make_hnln_ridge <- function(df_trait, trait_label) {
  ...
  ggplot(df_trait, aes(x = Value, y = ClusterName)) +
    ...
    labs(title = trait_label, x = "BLUE value", y = NULL, fill = NULL) +
...
out_pdf <- file.path(out_dir, "2020_2021_HN_LN_remaining_traits_ridge_4cols.pdf")
ggsave(out_pdf, combined_plot, width = 19, height = 16, units = "in", bg = "white")

file.copy(
  "/Users/subhashmahamkali/Documents/gwas_sap/profiling/01_publication/figure_2_racewise_phenotype/combine_2020_2021_hn_ln_remaining_traits.R",
  file.path(script_copy_dir, "combine_2020_2021_hn_ln_remaining_traits.R"),
  overwrite = TRUE
)
